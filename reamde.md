...
- **Продвинутая логика и валидация:**
    - **Обработка дубликатов ID:** Скрипт больше не прерывает работу при обнаружении дублирующихся или пустых ID. Вместо этого он выводит предупреждение (`WARN`) в консоль и пропускает дублирующую/некорректную строку, продолжая обработку остальных данных.
    - **Автоматическое создание сегментов:**
        - **Из шаблонов-сегментов:** Если на листе "Сегменты" у одного сегмента в поле "Расположение" указано несколько ЦОДов, скрипт использует эту строку как шаблон. Он создает новые сегменты для каждого дополнительного ЦОДа, копируя "Зону" и адаптируя "Наименование" и "ID".
        - **Из шаблонов-сетей:** Если на листе "Сети" у сети указан один сегмент, но несколько расположений, скрипт автоматически создает недостающие сегменты в этих расположениях и **автоматически привязывает их к данной сети**.
    - **Проверка согласованности расположений:** Добавлена новая проверка в `validate_refs`. Если у сети явно указано расположение, скрипт проверяет, что все сегменты, к которым она привязана, также существуют в этом расположении. В противном случае выводится ошибка, например: `Network 'Net-123' is in location 'dc01', but none of its associated segments exist in that location.`
