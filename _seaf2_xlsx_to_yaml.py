...
def convert_regions_az_dc_offices(xlsx_path: Path, out_dir: Path):
    ...
    if regions:
        write_yaml(out_dir / 'dc_region.yaml', {'seaf.company.ta.services.dc_regions': regions})
    ...
    if azs:
        write_yaml(out_dir / 'dc_az.yaml', {'seaf.company.ta.services.dc_azs': azs})
    ...
    if dcs:
        write_yaml(out_dir / 'dc.yaml', {'seaf.company.ta.services.dcs': dcs})
    ...
    if offices:
        write_yaml(out_dir / 'dc_office.yaml', {'seaf.company.ta.services.dc_offices': offices})
...
def write_networks_per_location(nets: Dict[str, Any], out_dir: Path, company_prefix: str):
    ...
    for token, subset in per_loc.items():
        ...
        write_yaml(out_dir / fname, {'seaf.company.ta.services.networks': subset})
        written.append(fname)
    if misc:
        fname = 'networks_misc.yaml'
        write_yaml(out_dir / fname, {'seaf.company.ta.services.networks': misc})
        written.append(fname)
    return written
...
def convert_segments_nets_devices(xlsx_path: Path, out_dir: Path) -> int:
    ...
    if segments:
        write_yaml(out_dir / 'network_segment.yaml', {'seaf.company.ta.services.network_segments': segments})
    if nets:
    ...
    if devices:
        write_yaml(out_dir / 'network_component.yaml', {'seaf.company.ta.components.networks': devices})
...
def convert_kb_services(xlsx_path: Path, out_dir: Path):
    ...
    if kb_services:
        write_yaml(out_dir / 'kb.yaml', {'seaf.company.ta.services.kbs': kb_services})

def write_root(out_dir: Path):
    imports = [p.name for p in sorted(out_dir.glob('*.yaml')) if p.name != 'root.yaml']
    if imports:
        write_yaml(out_dir / '_root.yaml', {'imports': imports})
...
def validate_refs(out_dir: Path) -> Dict[str, Any]:
    ...
    try:
        regions = load_if_exists('dc_region.yaml').get('seaf.company.ta.services.dc_regions', {})
        azs = load_if_exists('dc_az.yaml').get('seaf.company.ta.services.dc_azs', {})
        dcs = load_if_exists('dc.yaml').get('seaf.company.ta.services.dcs', {})
        offices = load_if_exists('dc_office.yaml').get('seaf.company.ta.services.dc_offices', {})
        segments = load_if_exists('network_segment.yaml').get('seaf.company.ta.services.network_segments', {})
        devices = load_if_exists('network_component.yaml').get('seaf.company.ta.components.networks', {})
        kb_services = load_if_exists('kb.yaml').get('seaf.company.ta.services.kbs', {})
        
        networks: Dict[str, Any] = {}
        for p in sorted(out_dir.glob('network*.yaml')):
            if p.name != 'network_component.yaml' and p.name != 'network_segment.yaml':
                networks.update(load_if_exists(p.name).get('seaf.company.ta.services.networks', {}))
